...
class Instructions:
    ...
    def add(self, ctx, listener, variables):
        ...
        for arg in ctx.argument():
            if arg.expression() is not None and arg.expression().math(
...
class VusListener(CorListener):
    ...
    def assignment_arr(self, ctx):
        ...
        if num_dimensions == 1:
            if ctx.array().string() is not None:
                ...
                width = math.ceil(
                    (ctx.array().arr_data().getChildCount() - 2) / 2.0)
                for i in range(width):

                    linenum = self.stream.get(ctx.array().arr_data().getChild(
                        1 + i * 2).getSourceInterval()[0]).line
    ...
    def declaration(self, ctx):
        ...
        if ctx.getChildCount() == 2:  # simple declaration
            self.variables.add(ctx.RAM().getText(),
    ...
    def exitStatement_loop(self, ctx: CorParser.Statement_loopContext):
        self.instructions.addLoop(ctx.loop(),
                                  self,
                                  self.variables,
                                  self.labels,
                                  top=True)
