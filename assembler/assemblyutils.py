...
def parseInput(argv):
    ...
    for i in range(length - 1, -1, -1):
        if tempargs[i] in options_args:
            if i == length - 1 or tempargs[i + 1] in options_args or tempargs[
                    i + 1] in options_noargs:
                print(f"Error: expected argument after option {tempargs[i]}",
                      end='\n\n')
                sys.exit(1)
...
def writeMem(code, outfile, memAddBits, memBits):
        ...
        for i in range(numLines):
            ...
            for j in range(wordsPerLine):
                if j + i * wordsPerLine < len(code):
                    file.write('{:0>{w}X} '.format(code[j + i * wordsPerLine],
                                                   w=memWidth))
                else:
    ...
    if len(code) > 2**memAddBits:
        print('Error: program size too big ({} words) for memory ({} words)'.
              format(len(code), 2**memAddBits))
        sys.exit(1)
                ...
                for line in instructions.getInstructions():
                    ...
                    for i in range(len(line['arguments'])):
                        ...
                        if i < len(line['arguments']) - 1:
                            tempargs += ', '
                    file.write(
                        f"{line['address']} : {line['mnemonic']} {tempargs}\n")
...
def endExecution():
    if len(errorList) > 0:
        ...
        print(
            f'{len(warningList)} warning{warnS}, {len(errorList)} error{errS}, exiting...'
        )
        sys.exit(1)
    if len(warningList) > 0:
        ...
        errS = 's' if len(errorList) != 1 else ''
        print(
            f'{len(warningList)} warning{warnS}, {len(errorList)} error{errS}')


def error(message, lineNumber, filepath, errorcode,
          syntax=False, col=0, abort=False):
    errorList.append({
        'message': message,
        'line': lineNumber,
        'path': filepath,
    ...
    for entry in entries:
        try:
            with open(entry['path'], 'r') as file:
                ...
                if entry['syntax']:
                    print(
                        f'({entry["code"]}) Error in file {bcolors.green}\"{entry["path"]}\"{bcolors.stop} at line {entry["line"]}:'
                    )
                    original = lines[entry['line'] - 1].rstrip(' \n')
                    print(f'  {original}')
                    print(f'  {" "*entry["col"]}{bcolors.red}^{bcolors.stop}')
                    print(
                        f'{bcolors.blue}->{bcolors.stop} {entry["message"]}\n')
                else:
                    print(
                        f'({entry["code"]}) {bcolors.red}Error{bcolors.stop} in file {bcolors.green}\"{entry["path"]}\"{bcolors.stop} at line {entry["line"]}:'
                    )
...
def warningPrint(entries):
    if len(entries) > 0:
        with open(entries[0]['path'], 'r') as file:
            ...
            for entry in entries:
                print(
                    f'({entry["code"]}) Warning for file \"{entry["path"]}\" at line {entry["line"]}:'
                )
                original = lines[entry['line'] - 1].strip(' \n')
                print(f'  {original}')
                print(f'-> {entry["message"]}\n')
    ...
    for label in labels:
        if 'interrupt' in label:
            try:
                ...
                tempargs = instructions[INTERRUPT_VECTORS[
                    label['interrupt']]]['arguments']
                ...
                instructions[INTERRUPT_VECTORS[
                    label['interrupt']]]['arguments'] = [label['name']]
            except KeyError:
...
def assembleArgument2Store(word, argument, dict, instruction, variables,
                           number):
    validTable = ['ram', 'gpu']
...
def assembleInstructions(instructions, variables, labels):
    ...
    mathops = [
        'cmp', 'add', 'sub', 'mul', 'div', 'mod', 'and', 'or', 'xor', 'lsr',
        'lsl'
    ]
    ...
    for i in range(len(instructions)):
        ...
        if inst in mathops:
            if len(args) < 2 or len(args) > 3:
                ...
                error(errmess, instructions[i]['line'],
                      instructions[i]['path'], 76)

            try:
                word = assembleRegister(word, args[0], 'operand1',
                                        instructions[i])
                ...
                try:
                    ...
                    word = assembleArgument2(word, args[1], MATH_DICT,
                                             instructions[i], variables, number)
                if len(args) == 3:
                    if 'inst' == 'cmp':
                        errmess = f'\"{inst}\" takes exactly two arguments, ignoring \"{args[2]}\"'
                        warning(errmess, instructions[i]['line'],
                                instructions[i]['path'], 119)
                    word = assembleRegister(word, args[2], 'results',
                                            instructions[i])
                else:
                    word = assembleRegister(word, args[0], 'results',
                                            instructions[i])
            except IndexError:
                pass
            ...
            if len(args) < 1 or len(args) > 2:
                ...
                error(errmess, instructions[i]['line'],
                      instructions[i]['path'], 169)
            try:
                word = assembleRegister(word, args[0], 'operand2',
                                        instructions[i])
                word = setOpvar(word, MATH_DICT['reg'])
                if len(args) == 2:
                    word = assembleRegister(word, args[1], 'results',
                                            instructions[i])
                else:
                    word = assembleRegister(word, args[0], 'results',
                                            instructions[i])
            ...
            try:
                ...
                word = assembleArgument2(word, args[1], LOAD_DICT,
                                         instructions[i], variables, number)
            except IndexError:
                pass
            ...
            try:
                word = assembleRegister(word, args[0], 'operand1',
                                        instructions[i])
                word = assembleArgument2Store(word, args[1], STORE_DICT,
                                              instructions[i], variables,
                                              number)
            except IndexError:
                pass
...
def assembleVariables(variables, ramBitWidth):
    biggestAddress = -1
    for var in variables:
        if variables[var]['type'] == 'rom' and int(
                variables[var]['address']) > biggestAddress:
